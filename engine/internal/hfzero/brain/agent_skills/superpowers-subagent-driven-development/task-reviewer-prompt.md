...
Use this template when delegating a task reviewer sub-agent. The reviewer
...
```
delegate tool call:
  task: "Review Task N (spec + quality)"
  ...
  prompt: |
    ...
    **Diff file:** [DIFF_FILE]
    ...
    ## You Do Not Delegate Sub-agents
    ...
    delegate), check the diff against that list file by file: every listed
...
- `[BRIEF_FILE]` — REQUIRED: the task brief file (the controller extracted
  the task text from the plan to `<workspace>/task-N-brief.md`; same file the
...
- `[DIFF_FILE]` — REQUIRED: the path the controller wrote the review
  package to (git log + git diff --stat + git diff -U10 for the range,
  redirected to one uniquely named workspace file; the package never enters
