...
func newMockHF(t *testing.T) *mockHF {
        ...
        // discovery by tag / search / author (v0.48 channels).
        mux.HandleFunc("GET /api/datasets", func(w http.ResponseWriter, r *http.Request) {
                q := r.URL.Query()
                filter, search, author := q.Get("filter"), q.Get("search"), q.Get("author")
                m.mu.Lock()
                defer m.mu.Unlock()
                var out []map[string]any
                for id, repo := range m.repos {
                        if filter != "" && !mockHasTag(repo, filter) {
                                continue
                        }
                        if search != "" && !strings.Contains(id, search) {
                                continue
                        }
                        if author != "" && !strings.HasPrefix(id, author+"/") {
                                continue
                        }
        ...
        mux.HandleFunc("GET /api/datasets/{user}/{name}", func(w http.ResponseWriter, r *http.Request) {
                m.mu.Lock()
                defer m.mu.Unlock()
                repo, ok := m.repos[r.PathValue("user")+"/"+r.PathValue("name")]
                if !ok {
                        w.WriteHeader(http.StatusNotFound)
                        return
                }
                writeMockJSON(w, map[string]any{"id": r.PathValue("user") + "/" + r.PathValue("name"), "tags": repo.tags, "private": false, "author": r.PathValue("user"), "lastModified": "2026-09-01T00:00:00.000Z"})
        })
        ...
        mux.HandleFunc("GET /datasets/{user}/{name}/resolve/main/{path...}", func(w http.ResponseWriter, r *http.Request) {
                repo, path := r.PathValue("user")+"/"+r.PathValue("name"), r.PathValue("path")
                m.mu.Lock()
                rp, ok := m.repos[repo]
                m.mu.Unlock()
                if !ok {
                ...
                if repo == "redir/doomalay-personas" && path == "items/index.json" {
                        http.Redirect(w, r, "/datasets/redir/doomalay-personas/resolve/main/items/real-index.json", http.StatusTemporaryRedirect)
                        return
                }
                m.mu.Lock()
                defer m.mu.Unlock()
        ...
        // tree listing (index fallback + v0.48 scanner probes; emits both
        // files and immediate subdirectories, like the real HF tree API).
        mux.HandleFunc("GET /api/datasets/{user}/{name}/tree/main/{dir...}", func(w http.ResponseWriter, r *http.Request) {
                dir := r.PathValue("dir")
                m.mu.Lock()
                defer m.mu.Unlock()
                rp, ok := m.repos[r.PathValue("user")+"/"+r.PathValue("name")]
                if !ok {
                        w.WriteHeader(http.StatusNotFound)
                        return
                }
                ...
                for path := range rp.files {
                        ...
                        if i := strings.IndexByte(rest, '/'); i >= 0 {
                                ...
                                if !seenDirs[d] {
                                        seenDirs[d] = true
                                        out = append(out, map[string]any{"type": "directory", "path": d, "size": 0})
                                }
                                continue
                        }
        ...
        mux.HandleFunc("POST /api/datasets/{user}/{name}/commit/main", func(w http.ResponseWriter, r *http.Request) {
                ...
                defer m.mu.Unlock()
                mockDeriveTags(rp)
                writeMockJSON(w, map[string]any{"commitUrl": "/" + r.PathValue("user") + "/" + r.PathValue("name") + "/commit/mocksha"})
        })
