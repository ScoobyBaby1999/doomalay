...
func chatProbeResult(provider string, cfg ProviderConfig, apiKey, accountID, probeModel string) (ValidateResult, bool) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "context deadline exceeded") || strings.Contains(err.Error(), "Timeout") {
			reason = "provider slow to answer (their queue) — key saved; chat usually still works"
		}
		return ValidateResult{State: "unverified", Reason: reason}, false
	}
	...
	case status == 401 || status == 403:
		// v0.35 (2026-09 live-verified with the user's key): OpenCode's
		// FreeTierError means the KEY IS VALID — the free models are just
		// walled to OpenCode's own client (the same key on a PAID model
		// answers 402 "insufficient funds", i.e. auth PASSED). Treating it
		// as "invalid" dead-ended the whole connect flow and auto-pick.
		if strings.Contains(bodyStr, "FreeTierError") || strings.Contains(bodyStr, "free tier can only be used") {
			return ValidateResult{State: "valid", Valid: true, Reason: "key is valid — OpenCode's FREE models are restricted to the OpenCode app itself; add credits at opencode.ai to use the paid models here"}, true
		}
...
// v0.44: "<provider>/auto" (the brain's /models pseudo-entry, which the
// provider auto-pick flow can store on a session) resolves to the
// provider's BEST concrete model from the v2 catalog here — the turn
// pipelines never send a literal "auto" to a provider API again.
func ResolveModel(userModel, userProvider string, keys map[string]string) (model, baseURL, envVar, apiKey, authStyle string, err error) {
	...
	// v0.44 AUTO RESOLUTION: swap the "<provider>/auto" pseudo-model
	// for the provider's best concrete model (free + popular families
	// first). The brain path resolves the same way in server/chat.go
	// BEFORE the request leaves the engine.
	if r := ResolveAutoModel(userModel, userProvider, keys); r != "" {
		userModel = r
	}
...
// ── v0.44 AUTO-MODEL RESOLUTION ──────────────────────────────────────────
// The brain's /models serves "<provider>/auto" pseudo-entries (one per
// keyed provider) and the provider connect flow can store one on a
// session. A literal "auto" 404s at every provider API and CRASHED the
// brain's /chat with an IndexError (p.models[0] on an empty tuple —
// providers_catalog.json's model lists are stale). The ENGINE owns the
...
// ("nvidia/z-ai/glm-5.3-flash") when userModel is an auto pseudo-model
// ("<provider>/auto" or bare "auto"), or "" when nothing needs resolving
// (or the catalog has no models for the provider — the caller keeps the
// original and the failure surfaces honestly).
func ResolveAutoModel(userModel, userProvider string, keys map[string]string) string {
	if userProvider == "" {
		return ""
	}
	if userModel != "auto" && userModel != userProvider+"/auto" {
		return ""
	}
	...
	for _, g := range cat.Groups {
		if g.Name != userProvider || len(g.Models) == 0 {
			continue
		}
		best, bestScore := "", -1
