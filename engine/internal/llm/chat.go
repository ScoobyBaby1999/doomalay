...
// can reach for live facts on ANY turn. Path A (OpenRouter native search)
// still only runs when req.WebSearch is on.
...
func runWebSearchTurn(ctx context.Context, ch chan<- ChatChunk, errs chan<- error, req ChatRequest) {
	...
	// v0.44 SELF-ENABLE (user spec: tools the model can just USE):
	// webToolsProtocol is now ALWAYS injected — the model may call
	// web_search/web_fetch whenever it needs live facts (the old
...
func actionHasRequiredArg(name, argJSON string) bool {
	...
	case "template_show":
		...
		return json.Unmarshal([]byte(argJSON), &a) == nil && strings.TrimSpace(a.ID) != ""
	...
	return true // no required arg (time_now, uuid, persona_list, template_list…) or unknown tool
}
