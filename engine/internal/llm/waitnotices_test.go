...
// produce live "waiting on <provider> · Ns" progress chunks so the UI never
...
func TestFriendlyHTTPErrors(t *testing.T) {
        cases := []struct {
                ...
                // v0.35: end-of-life models say it plainly.
                {410, `{"detail":"The model 'deepseek-ai/deepseek-v4-flash' has reached its end of life"}`, "retired by the provider"},
        }
        for _, c := range cases {
