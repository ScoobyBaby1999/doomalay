...
func newMockHubHF(t *testing.T) *mockHubHF {
	...
	mux.HandleFunc("GET /api/datasets/{repo}", func(w http.ResponseWriter, r *http.Request) {
		m.mu.Lock()
		defer m.mu.Unlock()
		if _, ok := m.repos[r.PathValue("repo")]; !ok {
			w.WriteHeader(http.StatusNotFound)
			return
		}
		writeHubMockJSON(w, map[string]any{"id": r.PathValue("repo"), "private": false})
	})

	mux.HandleFunc("GET /datasets/{repo}/resolve/main/{path...}", func(w http.ResponseWriter, r *http.Request) {
		m.mu.Lock()
		defer m.mu.Unlock()
		files, ok := m.repos[r.PathValue("repo")]
		if !ok {
			w.WriteHeader(http.StatusNotFound)
			return
		}
	...
	mux.HandleFunc("POST /api/datasets/{repo}/commit/main", func(w http.ResponseWriter, r *http.Request) {
		...
		mockHubDeriveTags(m, r.PathValue("repo"))
		writeHubMockJSON(w, map[string]any{"commitUrl": "/" + r.PathValue("repo") + "/commit/mocksha"})
	})
