// icons.js — v0.52 THE ICON LIBRARY (the hub cards' icon column, the
// collection bunches, the publish form's picker).
//
// USER SPEC (item 3): "please add an icon library of 30+ icons the user
// can choose from, and have each card have a column for the icon left of
// the name or title, icons are optional, users can opt to have none".
//
// The glyphs are Lucide v1.47.0 (ISC License — see docs/CREDITS.md),
// embedded as inline SVG so the whole set works OFFLINE (the engine
// serves the PWA from a single binary; no CDN, no requests). Every icon
// strokes with currentColor → it follows the app theme + the surface it
// sits on for free. Corpus rows already carry these names ("icon":
// "lightbulb" | "bug" | "map" | "layers" | "flask" | "shield" | "zap").
//
// Exposes: window.IconLib = { NAMES, has, svg, el }
//   IconLib.svg(name, size?)  → '<svg …>' string ('' when unknown)
//   IconLib.el(name, size?)   → a live HTMLElement (span > svg)
(function () {
  'use strict';
  ...
  // svg(name, size?) — the themed glyph as a string. Unknown/empty name
  // returns '' (icons are optional; callers render nothing).
  function svg(name, size) {
    ...
    return '<svg class="icl icl-' + String(name).replace(/[^a-z0-9-]/g, '') +
      ...
      ICONS[String(name)] + '</svg>';
  }

  // el(name, size?) — a live <span class="icl-wrap"> wrapping the svg,
  // ready to drop into a card's icon column.
  function el(name, size) {
    var s = svg(name, size);
    if (!s) return null;
    var w = document.createElement('span');
    w.className = 'icl-wrap';
    w.innerHTML = s;
    return w;
  }
