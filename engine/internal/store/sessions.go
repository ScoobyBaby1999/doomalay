...
INSERT INTO chat_sessions
  (id, title, model, provider, effort, mode, web_search, deep_research,
...
VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		s.ID, s.Title, s.Model, s.Provider, s.Effort, s.Mode,
...
UPDATE chat_sessions SET
  model=?, provider=?, effort=?, mode=?, web_search=?, deep_research=?,
  web_template=?, deep_template=?, deep_mode=?, judge_count=?, judge_template=?,
  sliding_window=?, max_context=?, tool_allowlist=?, hooks_config=?,
  routing=?, workspace_id=?, updated_at=?
WHERE id=?`,
		s.Model, s.Provider, s.Effort, s.Mode, s.WebSearch, s.DeepResearch,
...
func (db *DB) GetSession(id string) (*Session, error) {
	...
	var ws, dr, mr int
	err := db.QueryRow(`
SELECT id, title, model, provider, effort, mode, web_search, deep_research,
       web_template, deep_template, deep_mode, judge_count, judge_template,
       sliding_window, max_context, tool_allowlist, hooks_config, routing,
       workspace_id, manually_renamed, created_at, updated_at
FROM chat_sessions WHERE id=?`, id).Scan(
		&s.ID, &s.Title, &s.Model, &s.Provider, &s.Effort, &s.Mode, &ws, &dr,
		&s.WebTemplate, &s.DeepTemplate, &s.DeepMode, &s.JudgeCount, &s.JudgeTemplate,
