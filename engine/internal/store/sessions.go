...
func (db *DB) GetSession(id string) (*Session, error) {
	s := &Session{}
	var ws, dr, mr int
	var persona, personas, placeholders sql.NullString
	err := db.QueryRow(`
SELECT id, title, model, provider, sandbox, effort, mode, web_search, deep_research,
       web_template, deep_template, deep_mode, judge_count, judge_template,
       sliding_window, max_context, tool_allowlist, hooks_config, routing,
       workspace_id, persona, personas, placeholders, manually_renamed, created_at, updated_at
FROM chat_sessions WHERE id=?`, id).Scan(
		...
		&s.WorkspaceID, &persona, &personas, &placeholders, &mr, &s.CreatedAt, &s.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
	if err != nil {
