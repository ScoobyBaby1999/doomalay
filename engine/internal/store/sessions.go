...
type Session struct {
        ...
        //   "own"    → SandboxRepo ("user/name"), auth: the space token in
        //              the vault (HF_SPACE_<OWNER>_<NAME>)
        SandboxMode string
        SandboxRepo string
        CreatedAt   float64
        UpdatedAt   float64
...
FROM chat_sessions WHERE id=?`, id).Scan(
        ...
        if sandboxMode.Valid {
                s.SandboxMode = sandboxMode.String
        }
        if sandboxRepo.Valid {
                s.SandboxRepo = sandboxRepo.String
        }
        return s, nil
}
